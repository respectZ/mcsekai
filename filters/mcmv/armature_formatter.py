    ...
    def set_bones(self, visible_bone_list: list[Union[tuple[str, str, Vector3, Vector3, DisplayVoxel], tuple[str, str]]]):
        ...
        for visible_bone_tuple in visible_bone_list:
            if len(visible_bone_tuple) == 5:
                ...
                new_bone = VisibleBone(name, size.scaled_pixels_to_meter(
                ), offset.scaled_pixels_to_meter(), display)
            else:
        ...
        def dfs(joint: Joint, reached_base: bool) -> Bone:
            if reached_base:
                ...
                if d[1] == 'x':
                    ...
                    bone_size_offset_vector = Vector3(
                        0.0, -width / 2, -width / 2)
                elif d[1] == 'y':
        ...
        def dfs(bone: Bone):
            ...
            for child_name in bone.children:
                ...
                global_transformation[child_name] = (
                    child_global_offset, child_global_rotation)
...
class ArmatureFormatter:
    ...
    def get_model_global(model: ArmatureModel) -> dict[str, tuple[Vector3, Quaternion]]:
        global_transformation = {model.root.name: (
            model.root.animation_offset, Quaternion())}
        ...
        def dfs(joint: Joint):
            ...
            for child_name in joint.children:
                ...
                child_translation = parent_translation + child.animation_offset.rotated_by_quaternion(
                    parent_rotation)  # TODO rotate this by grandparent rotation
                child_rotation = child.animation_rotation.parented(
                    parent_rotation)
